...
class BaseCase(object):
    ...
    def make_user(self, log_cor=True, psw_cor=True, email_cor=True, rand_code=False):
        ...
        resp = requests.Session().get(f'http://127.0.0.1:5000/vk_id/{self.log}')
        if '404' in resp.json()['Status']:
            resp = requests.Session().post(f'http://127.0.0.1:5000/new_user/{self.log}')
        return resp.json()['vk_id']
